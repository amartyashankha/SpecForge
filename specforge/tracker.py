...
class WandbTracker(Tracker):
    ...
    Supports run resumption via WANDB_RUN_ID/WANDB_RESUME env vars (set by Modal entrypoint).
    """
    ...
    def __init__(self, args, output_dir: str):
        ...
        if self.rank == 0:
            ...
            # wandb.init() picks up WANDB_RUN_ID and WANDB_RESUME from env (set by Modal)
            wandb.init(
            ...
            if wandb.run:
                print(f"[WANDB] {wandb.run.url} (resumed={wandb.run.resumed})", flush=True)
